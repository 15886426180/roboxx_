...
#if ENABLE_WORKER != 0
    counting_semaphore<FRAME_BUFFER> image_to_process(0);
    counting_semaphore<FRAME_BUFFER> image_ready_to_copy(FRAME_BUFFER);
    ticket_mutex              image_lock;
    ticket_mutex              send_lock;

    std::jthread read_image([&]() -> void {
        Mat _thr_tmp(frame.size(), CV_8UC3);
        while(1) {
            image_ready_to_copy.acquire();
            if (cap.isindustryimgInput())
                 _thr_tmp = cvarrToMat(cap.iplImage, true);
            else
                capture >> frame;
            image_lock.lock();
            resize(_thr_tmp, frame, Size(640, 400));
            image_lock.unlock();
            image_to_process.release();
        }
    });

    std::jthread serial_send([&]() -> void { 
        while(1) {
            using namespace chrono_literals;
            auto t_s { chrono::steady_clock::now() };
            send_lock.lock();
...
#elif ROI_IMG == 0
    #if ENABLE_WORKER != 0
            image_to_process.acquire();

            image_lock.lock();
            src_img = frame;
            image_lock.unlock();

            image_ready_to_copy.release();
    #else
            src_img = frame;
    #endif
#endif
...
//串口传输
        ...
        this_thread::sleep_for(FRAME_LIMIT - (current - begin));
    #else
        //串口输出
        serial.RMserialWrite(_yaw, fabs(yaw)*100, _pitch, fabs(pitch)*100, depth, data_type, is_shooting);
    #endif
