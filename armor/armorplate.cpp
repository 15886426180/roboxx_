...
bool ImageProcess::light_Judge(int i, int j)
{
    ...
    if (armor_data.left_right_h < 1.9
            && armor_data.left_right_w > 0.6
            && armor_data.left_right_h > 0.6
            && armor_data.left_right_w < 1.9)
    {
        armor_data.height = (armor_data.right_light_height + armor_data.left_light_height) / 2.0f;
        // 两个灯条高度差不大
        if (fabs(armor_data.left_light.center.y - armor_data.right_light.center.y) < MIN(armor_data.right_light_height , armor_data.left_light_height)/2)
        {
